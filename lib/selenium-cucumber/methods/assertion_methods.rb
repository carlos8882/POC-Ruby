...
def check_element_presence(access_name, present)
    if present.empty?
      using_wait_time 15 do
        expect(page).to have_css("#{access_name}")
      end
    else
        expect(page).not_to have_css("#{access_name}")
    end
end

# method to check element exist
def check_element_exist(access_name)
  expect(page.has_css?(access_name)).to be true #raise_error("Element does not exist")
end

# def check_element_exist(access_name)
#   begin
#   # if page.has_css?(access_name)
#     element = get_element_text('css', access_name)
#     puts element
#   # puts element.exist
#   rescue
#     raise Exception.new "Element does not exist, Actual Value : #{access_name}"

#     # raise TestCaseFailed, "Element does not exist, Actual Value : #{access_name}"

#     # raise RuntimeError, "Element does not exist, Actual Value : #{access_name}"
#     # Capybara.raise_server_errors = true
#     # puts "Element does not exist, Actual Value : #{access_name}"
#     # expect(page).not_to have_css("#{access_name}")
#   end
# end
...
class TestCaseFailed < StandardError
